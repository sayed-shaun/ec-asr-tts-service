...
class ASREngine:
    ...
    def __init__(self, model_name: str, device: str = "auto", max_segment_seconds: float = 18.0):
    ...
    def transcribe(self, audios: list[np.ndarray], batch_size: int = 4, sample_rate: int = 16000) -> list[str]:
    ...
    def _split(audio: np.ndarray, max_segment_samples: int) -> list[np.ndarray]:
        ...
        if len(audio) <= max_segment_samples:
            return [audio]
        return [audio[start : start + max_segment_samples] for start in range(0, len(audio), max_segment_samples)]
